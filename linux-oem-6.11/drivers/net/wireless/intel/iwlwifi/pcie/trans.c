					...
					u32 byte_cnt)
{
	...
	ret = wait_event_timeout_dbg(trans_pcie->ucode_write_waitq,
...
static int iwl_pcie_d3_handshake(struct iwl_trans *trans, bool suspend)
{
	...
	ret = wait_event_timeout_dbg(trans_pcie->sx_waitq,
...
static void iwl_pcie_irq_set_affinity(struct iwl_trans *trans)
{
	...
	for (; i < iter_rx_q ; i++) {
		...
		ret = irq_set_affinity_hint_dbg(trans_pcie->msix_entries[i].vector,
			    ...
			    u32 dst_addr, u64 src_addr, u32 byte_cnt)
{
	...
	ret = wait_event_timeout_dbg(trans_pcie->imr_waitq,
