			   ...
			   bool update, unsigned int flags)
{
	...
	printk("%s %d : iwl_mvm_send_cmd : ADD_STA\n", __func__, __LINE__);
			       ...
			       int sta_id, u16 *queueptr, u8 tid)
{
	...
	printk("%s %d : iwl_mvm_send_cmd : SCD_QUEUE_CFG\n", __func__, __LINE__);
...
static int iwl_mvm_rm_sta_common(struct iwl_mvm *mvm, u8 sta_id)
{
	...
	printk("%s %d : iwl_mvm_send_cmd : REMOVE_STA\n", __func__, __LINE__);
				    ...
				    bool mcast)
{
	...
	status = ADD_STA_SUCCESS;
	printk("%s %d : iwl_mvm_send_cmd : ADD_STA_KEY\n", __func__, __LINE__);
				  ...
				  u16 buf_size, int baid)
{
	...
	printk("[MODULE -> %s], [THREAD -> %s] [iwl_mvm_send_cmd : WIDE_ID(DATA_PATH_GROUP, RX_BAID_ALLOCATION_CONFIG_CMD)] [%s] [%d]\n", THIS_MODULE->name, get_thread_name(), __func__, __LINE__);
				 ...
				 u8 sta_id, bool remove_key)
{
	...
	if (!iwl_mvm_has_new_rx_api(mvm)) {
		...
		printk("%s %d : iwl_mvm_send_cmd : MGMT_MCAST_KEY\n", __func__, __LINE__);
				   ...
				   u32 id)
{
	...
	printk("%s %d : iwl_mvm_send_cmd : WIDE_ID(MAC_CONF_GROUP, CANCEL_CHANNEL_SWITCH_CMD)\n", __func__, __LINE__);
