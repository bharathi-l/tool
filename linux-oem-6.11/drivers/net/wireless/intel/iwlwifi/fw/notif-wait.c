...
{
	spin_lock_init_dbg(&notif_wait->notif_wait_lock);
			   ...
			   struct iwl_rx_packet *pkt)
{
	...
	if (!list_empty(&notif_wait->notif_waits)) {
		...
		spin_lock_bh_dbg(&notif_wait->notif_wait_lock);
		...
		spin_unlock_bh_dbg(&notif_wait->notif_wait_lock);
	}
...
void iwl_abort_notification_waits(struct iwl_notif_wait_data *notif_wait)
{
	...
	wake_up_all_dbg(&notif_wait->notif_waitq);
}
			   ...
			   void *fn_data)
{
	...
	spin_lock_bh_dbg(&notif_wait->notif_wait_lock);
	list_add_dbg(&wait_entry->list, &notif_wait->notif_waits);
	spin_unlock_bh_dbg(&notif_wait->notif_wait_lock);
}
			  ...
			  unsigned long timeout)
{
	...
	ret = wait_event_timeout_dbg(notif_wait->notif_waitq,
