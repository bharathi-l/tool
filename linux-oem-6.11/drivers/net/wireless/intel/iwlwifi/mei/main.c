			       ...
			       const struct iwl_sap_nvm *sap_nvm)
{
	...
	wake_up_all_dbg(&mei->get_nvm_wq);
}
					   ...
					   const struct iwl_sap_msg_dw *dw)
{
	...
	wake_up_all_dbg(&mei->get_ownership_wq);
...
struct iwl_mei_nvm *iwl_mei_get_nvm(void)
{
	...
	ret = wait_event_timeout_dbg(mei->get_nvm_wq, mei->nvm, 2 * HZ);
	if (!ret)
		return NULL;
...
int iwl_mei_pldr_req(void)
{
	...
	for (i = 0; i < IWL_MEI_PLDR_NUM_RETRIES; i++) {
		...
		ret = wait_event_timeout_dbg(mei->pldr_wq, mei->pldr_active, HZ / 2);
		if (ret)
			break;
...
int iwl_mei_get_ownership(void)
{
	...
	ret = wait_event_timeout_dbg(mei->get_ownership_wq,
