			     ...
			     struct sta_info *sta)
{
	...
	ret = rhltable_remove_dbg(&local->sta_hash, &sta->hash_node,
			       sta_rht_params);
	return ret;
}
				  ...
				  struct link_sta_info *link_sta)
{
	...
	ret = rhltable_insert_dbg(&local->link_sta_hash,
				  ...
				  struct link_sta_info *link_sta)
{
	...
	ret  = rhltable_remove_dbg(&local->link_sta_hash,
			       &link_sta->link_hash_node, link_sta_rht_params);
	return ret;
}
			     ...
			     struct sta_info *sta)
{
	...
	ret = rhltable_insert(&local->sta_hash, &sta->hash_node,
			       sta_rht_params);
	return ret;
}
...
int sta_info_init(struct ieee80211_local *local)
{
	...
	err = rhltable_init_dbg(&local->sta_hash, &sta_rht_params);
	if (err)
		return err;

	err = rhltable_init_dbg(&local->link_sta_hash, &link_sta_rht_params);
	if (err) {
		rhltable_destroy_dbg(&local->sta_hash);
		return err;
	}
...
void sta_info_stop(struct ieee80211_local *local)
{
	...
	rhltable_destroy_dbg(&local->sta_hash);
	rhltable_destroy_dbg(&local->link_sta_hash);
}
