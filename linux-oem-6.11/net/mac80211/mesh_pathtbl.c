...
static void mesh_fast_tx_deinit(struct ieee80211_sub_if_data *sdata)
{
	...
	rhashtable_free_and_destroy_dbg(&cache->rht,
...
static void mesh_fast_tx_init(struct ieee80211_sub_if_data *sdata)
{
	...
	cache = &sdata->u.mesh.tx_cache;
	rhashtable_init_dbg(&cache->rht, &fast_tx_rht_params);
	INIT_HLIST_HEAD(&cache->walk_head);
	spin_lock_init_dbg(&cache->walk_lock);
}
...
static void mesh_table_init(struct mesh_table *tbl)
{
	...
	spin_lock_init_dbg(&tbl->gates_lock);
	spin_lock_init_dbg(&tbl->walk_lock);

	/* rhashtable_init_dbg() may fail only in case of wrong
	 * mesh_rht_params
	 */
	WARN_ON(rhashtable_init_dbg(&tbl->rhead, &mesh_rht_params));
}

static void mesh_table_free(struct mesh_table *tbl)
{
	rhashtable_free_and_destroy_dbg(&tbl->rhead,
...
void mesh_path_assign_nexthop(struct mesh_path *mpath, struct sta_info *sta)
{
	...
	spin_lock_irqsave_dbg(&mpath->frame_queue.lock, flags);
	...
	spin_unlock_irqrestore_dbg(&mpath->frame_queue.lock, flags);
}
				    ...
				    bool copy)
{
	...
	spin_lock_irqsave_dbg(&from_mpath->frame_queue.lock, flags);
	skb_queue_splice_init(&from_mpath->frame_queue, &failq);
	spin_unlock_irqrestore_dbg(&from_mpath->frame_queue.lock, flags);
...
int mesh_path_add_gate(struct mesh_path *mpath)
{
	...
	spin_lock_dbg(&tbl->gates_lock);
	hlist_add_head_rcu_dbg(&mpath->gate_list, &tbl->known_gates);
	spin_unlock_dbg(&tbl->gates_lock);
...
static void mesh_gate_del(struct mesh_table *tbl, struct mesh_path *mpath)
{
	...
	mpath->is_gate = false;
	spin_lock_bh_dbg(&tbl->gates_lock);
	hlist_del_rcu_dbg(&mpath->gate_list);
	mpath->sdata->u.mesh.num_gates--;
	spin_unlock_bh_dbg(&tbl->gates_lock);
				...
				const u8 *dst, gfp_t gfp_flags)
{
	...
	spin_lock_init_dbg(&new_mpath->state_lock);
...
{
	hlist_del_rcu_dbg(&entry->walk_list);
	rhashtable_remove_fast_dbg(&cache->rht, &entry->rhash, fast_tx_rht_params);
	kfree_rcu(entry, fast_tx.rcu_head);
}
		 ...
		 struct ieee80211_mesh_fast_tx_key *key)
{
	...
	if (!(entry->mpath->flags & MESH_PATH_ACTIVE) ||
		...
		spin_lock_bh_dbg(&cache->walk_lock);
		entry = rhashtable_lookup(&cache->rht, key, fast_tx_rht_params);
		if (entry)
		    mesh_fast_tx_entry_free(cache, entry);
		spin_unlock_bh_dbg(&cache->walk_lock);
		return NULL;
	}
			...
			struct sk_buff *skb, struct mesh_path *mpath)
{
	...
	if (unlikely(prev)) {
		...
		hlist_del_rcu_dbg(&prev->walk_list);
		kfree_rcu(prev, fast_tx.rcu_head);
	}

	hlist_add_head_dbg(&entry->walk_list, &cache->walk_head);

unlock_cache:
	spin_unlock_dbg(&cache->walk_lock);
unlock_sta:
				...
				const u8 *dst)
{
	...
	spin_lock_bh_dbg(&tbl->walk_lock);
	mpath = rhashtable_lookup_get_insert_fast(&tbl->rhead,
						  &new_mpath->rhash,
						  mesh_rht_params);
	if (!mpath)
		hlist_add_head_dbg(&new_mpath->walk_list, &tbl->walk_head);
	spin_unlock_bh_dbg(&tbl->walk_lock);
		 ...
		 const u8 *dst, const u8 *mpp)
{
	...
	ret = rhashtable_lookup_insert_fast(&tbl->rhead,
					    ...
					    mesh_rht_params);
	if (!ret)
		hlist_add_head_rcu_dbg(&new_mpath->walk_list, &tbl->walk_head);
	spin_unlock_bh_dbg(&tbl->walk_lock);
...
{
	hlist_del_rcu_dbg(&mpath->walk_list);
	rhashtable_remove_fast_dbg(&tbl->rhead, &mpath->rhash, mesh_rht_params);
 ...
 *
 * Locking: the function must me called within a rcu_read_lock_dbg region
 */
