...
static int iwl_mvm_mld_rm_sta_from_fw(struct iwl_mvm *mvm, u32 sta_id)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(MAC_CONF_GROUP, STA_REMOVE_CMD)\n", __func__, __LINE__);
				     ...
				     u32 lmac_id)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(MAC_CONF_GROUP, AUX_STA_CMD)\n", __func__, __LINE__);
				   ...
				   u16 *queueptr, u8 tid)
{
	...
	if (mvm->sta_remove_requires_queue_remove) {
		...
		pr_info("[MODULE -> %s], [THREAD -> %s] [WIDE_ID(DATA_PATH_GROUP,SCD_QUEUE_CONFIG_CMD)] [%s] [%d]\n", THIS_MODULE->name, get_thread_name(), __func__, __LINE__);
				       ...
				       bool disable)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(MAC_CONF_GROUP, STA_DISABLE_TX_CMD)\n", __func__, __LINE__);
					...
					u32 new_sta_mask)
{
	...
	for (baid = 0; baid < ARRAY_SIZE(mvm->baid_map); baid++) {
		...
		pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(DATA_PATH_GROUP, RX_BAID_ALLOCATION_CONFIG_CMD)\n", __func__, __LINE__);
