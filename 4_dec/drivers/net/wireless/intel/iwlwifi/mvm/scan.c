				   ...
				   struct cfg80211_sched_scan_request *req)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : SCAN_OFFLOAD_UPDATE_PROFILES_CMD\n", __func__, __LINE__);
...
static int iwl_mvm_lmac_scan_abort(struct iwl_mvm *mvm)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : SCAN_OFFLOAD_ABORT_CMD\n", __func__, __LINE__);
...
static int iwl_mvm_legacy_config_scan(struct iwl_mvm *mvm)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(IWL_ALWAYS_LONG_GROUP, SCAN_CFG_CMD)\n", __func__, __LINE__);
	ret = iwl_mvm_send_cmd(mvm, &cmd);
	if (!ret) {
		mvm->scan_type = type;
...
int iwl_mvm_config_scan(struct iwl_mvm *mvm)
{
	...
	pr_info("[MODULE -> %s], [THREAD -> %s] [iwl_mvm_send_cmd : WIDE_ID(IWL_ALWAYS_LONG_GROUP, SCAN_CFG_CMD)] [%s] [%d]\n", THIS_MODULE->name, get_thread_name(), __func__, __LINE__);
...
static int iwl_mvm_umac_scan_abort(struct iwl_mvm *mvm, int type)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(IWL_ALWAYS_LONG_GROUP, SCAN_ABORT_UMAC)\n", __func__, __LINE__);
