					...
					void *_data)
{
	...
	case WLAN_CIPHER_SUITE_WEP104: { /* hack it for now */
		...
		mutex_lock(&mvm->mutex);
		pr_info("%s %d : iwl_mvm_send_cmd : WEP_KEY\n", __func__, __LINE__);
					 ...
					 struct iwl_mvm_vif_link_info *mvm_link)
{
	...
	if (ver == 5) {
		...
		if (data.have_rsc) {
			pr_info("%s %d : iwl_mvm_send_cmd : WOWLAN_TSC_RSC_PARAM\n", __func__, __LINE__);
				    ...
				    struct cfg80211_wowlan *wowlan)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WOWLAN_PATTERNS\n", __func__, __LINE__);
				...
				struct ieee80211_sta *ap_sta)
{
	...
	status = 0;
	pr_info("%s %d : iwl_mvm_send_cmd : BINDING_CONTEXT_CMD\n", __func__, __LINE__);
	...
	pr_info("%s %d : iwl_mvm_send_cmd : TIME_QUOTA_CMD\n", __func__, __LINE__);
				       ...
				       struct ieee80211_vif *vif)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : NON_QOS_TX_COUNTER_CMD\n", __func__, __LINE__);
	err = iwl_mvm_send_cmd(mvm, &cmd);
	if (err)
		return err;
					    ...
					    struct iwl_mvm_vif_link_info *mvm_link)
{
	...
	if (!fw_has_api(&mvm->fw->ucode_capa,
		...
		if (tkip_data.have_tkip_keys) {
			...
			pr_info("%s %d : iwl_mvm_send_cmd : WOWLAN_TKIP_PARAM\n", __func__, __LINE__);
	...
	if (mvmvif->rekey_data.valid) {
		...
		pr_info("%s %d : iwl_mvm_send_cmd : WOWLAN_KEK_KCK_MATERIAL\n", __func__, __LINE__);
		      ...
		      struct ieee80211_sta *ap_sta)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WOWLAN_CONFIGURATION\n", __func__, __LINE__);
			     ...
			     bool test)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : D3_CONFIG_CMD\n", __func__, __LINE__);
...
iwl_mvm_send_wowlan_get_status(struct iwl_mvm *mvm, u8 sta_id)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WOWLAN_GET_STATUSES\n", __func__, __LINE__);
				...
				struct iwl_mvm_nd_results *results)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : SCAN_OFFLOAD_PROFILES_QUERY_CMD\n", __func__, __LINE__);
	ret = iwl_mvm_send_cmd(mvm, &cmd);
	if (ret) {
...
static int iwl_mvm_resume_firmware(struct iwl_mvm *mvm, bool test)
{
	...
	if (mvm->trans->trans_cfg->device_family <= IWL_DEVICE_FAMILY_22000 &&
		...
		pr_info("%s %d : iwl_mvm_send_cmd : D0I3_END_CMD\n", __func__, __LINE__);
		ret = iwl_mvm_send_cmd(mvm, &cmd);
		if (ret < 0)
