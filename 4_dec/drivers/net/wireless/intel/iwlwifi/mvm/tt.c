...
static int iwl_mvm_send_temp_cmd(struct iwl_mvm *mvm, bool response, s32 *temp)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(PHY_OPS_GROUP, CMD_DTS_MEASUREMENT_TRIGGER_WIDE)\n", __func__, __LINE__);
...
void iwl_mvm_tt_tx_backoff(struct iwl_mvm *mvm, u32 backoff)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : REPLY_THERMAL_MNG_BACKOFF\n", __func__, __LINE__);
...
int iwl_mvm_ctdp_command(struct iwl_mvm *mvm, u32 op, u32 state)
{
	...
	status = 0;
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(PHY_OPS_GROUP,CTDP_CONFIG_CMD)\n", __func__, __LINE__);
