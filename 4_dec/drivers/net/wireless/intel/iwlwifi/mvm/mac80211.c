			 ...
			 s16 tx_power)
{
	...
	pr_info("[MODULE -> %s], [THREAD -> %s] [iwl_mvm_send_cmd : REDUCE_TX_POWER_CMD] [%s] [%d]\n", THIS_MODULE->name, get_thread_name(), __func__, __LINE__);
				  ...
				  struct ieee80211_bss_conf *link_conf)
{
	...
	if (!fw_has_capa(&mvm->fw->ucode_capa,
		...
		pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(MAC_CONF_GROUP,CHANNEL_SWITCH_TIME_EVENT_CMD)\n", __func__, __LINE__);
				      ...
				      struct ieee80211_vif *vif)
{
	...
	pr_info("[MODULE -> %s], [THREAD -> %s] [iwl_mvm_send_cmd : MCAST_FILTER_CMD] [%s] [%d]\n", THIS_MODULE->name, get_thread_name(), __func__, __LINE__);
...
int iwl_mvm_update_mu_groups(struct iwl_mvm *mvm, struct ieee80211_vif *vif)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(DATA_PATH_GROUP,UPDATE_MU_GROUPS_CMD)\n", __func__, __LINE__);
			       ...
			       struct ieee80211_vif *vif, u8 sta_id)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(DATA_PATH_GROUP, STA_HE_CTXT_CMD)\n", __func__, __LINE__);
				    ...
				    int duration)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : HOT_SPOT_CMD\n", __func__, __LINE__);
				     ...
				     const void *data, u32 size)
{
	...
	pr_info("%s %d : iwl_mvm_send_cmd : WIDE_ID(DATA_PATH_GROUP, TRIGGER_RX_QUEUES_NOTIF_CMD)\n", __func__, __LINE__);
	ret = iwl_mvm_send_cmd(mvm, &hcmd);
	if (ret) {
