#include <linux/module.h>
#define INCLUDE_VERMAGIC
#include <linux/build-salt.h>
#include <linux/elfnote-lto.h>
#include <linux/export-internal.h>
#include <linux/vermagic.h>
#include <linux/compiler.h>

#ifdef CONFIG_UNWINDER_ORC
#include <asm/orc_header.h>
ORC_HEADER;
#endif

BUILD_SALT;
BUILD_LTO_INFO;

MODULE_INFO(vermagic, VERMAGIC_STRING);
MODULE_INFO(name, KBUILD_MODNAME);

__visible struct module __this_module
__section(".gnu.linkonce.this_module") = {
	.name = KBUILD_MODNAME,
	.init = init_module,
#ifdef CONFIG_MODULE_UNLOAD
	.exit = cleanup_module,
#endif
	.arch = MODULE_ARCH_INIT,
};

MODULE_INFO(intree, "Y");

#ifdef CONFIG_MITIGATION_RETPOLINE
MODULE_INFO(retpoline, "Y");
#endif

KSYMTAB_DATA(nf_nat_sip_hooks, "_gpl", "");
KSYMTAB_FUNC(ct_sip_parse_request, "_gpl", "");
KSYMTAB_FUNC(ct_sip_get_header, "_gpl", "");
KSYMTAB_FUNC(ct_sip_parse_header_uri, "_gpl", "");
KSYMTAB_FUNC(ct_sip_parse_address_param, "_gpl", "");
KSYMTAB_FUNC(ct_sip_parse_numerical_param, "_gpl", "");
KSYMTAB_FUNC(ct_sip_get_sdp_header, "_gpl", "");

SYMBOL_CRC(nf_nat_sip_hooks, 0x183be24a, "_gpl");
SYMBOL_CRC(ct_sip_parse_request, 0xdef5a181, "_gpl");
SYMBOL_CRC(ct_sip_get_header, 0x428f29ef, "_gpl");
SYMBOL_CRC(ct_sip_parse_header_uri, 0x6be20a40, "_gpl");
SYMBOL_CRC(ct_sip_parse_address_param, 0xf3663533, "_gpl");
SYMBOL_CRC(ct_sip_parse_numerical_param, 0xa6bdbbfa, "_gpl");
SYMBOL_CRC(ct_sip_get_sdp_header, 0xd34b004a, "_gpl");

static const struct modversion_info ____versions[]
__used __section("__versions") = {
	{ 0x9adb7399, "nf_conntrack_expect_lock" },
	{ 0xc3690fc, "_raw_spin_lock_bh" },
	{ 0x62a90113, "nf_ct_remove_expect" },
	{ 0xe46021ca, "_raw_spin_unlock_bh" },
	{ 0x89d99ee1, "__nf_ct_ext_find" },
	{ 0x55385e2e, "__x86_indirect_thunk_r14" },
	{ 0xa15cbb7c, "nf_ct_helper_log" },
	{ 0xaf9ba12f, "__nf_ct_refresh_acct" },
	{ 0x68946dc7, "__pskb_pull_tail" },
	{ 0xf5d586df, "skb_copy_bits" },
	{ 0xc28997d7, "nf_ct_expect_alloc" },
	{ 0xadb85c52, "nf_ct_expect_init" },
	{ 0x286b5af5, "nf_ct_expect_put" },
	{ 0x641eb8, "nf_ct_expect_related_report" },
	{ 0x289502a7, "__nf_ct_expect_find" },
	{ 0xcead911, "__nf_ip6_route" },
	{ 0x986d6241, "dst_release" },
	{ 0x3599c47a, "nf_ip_route" },
	{ 0x56cc812a, "nf_ct_unexpect_related" },
	{ 0x15ba50a6, "jiffies" },
	{ 0x1e0a0c24, "mod_timer_pending" },
	{ 0x47d0e073, "param_ops_int" },
	{ 0xd786c3ef, "param_ops_uint" },
	{ 0xaa473cb8, "param_array_ops" },
	{ 0x2f42db77, "param_ops_ushort" },
	{ 0xbdfb6dbb, "__fentry__" },
	{ 0x5b8239ca, "__x86_return_thunk" },
	{ 0x11089ac7, "_ctype" },
	{ 0x96b29254, "strncasecmp" },
	{ 0x609bcd98, "in6_pton" },
	{ 0xac5fcec0, "in4_pton" },
	{ 0xf0fdf6cb, "__stack_chk_fail" },
	{ 0x2cf56265, "__dynamic_pr_debug" },
	{ 0x20000329, "simple_strtoul" },
	{ 0x754d539c, "strlen" },
	{ 0x5a921311, "strncmp" },
	{ 0x65487097, "__x86_indirect_thunk_rax" },
	{ 0x602f99c4, "nf_ct_helper_init" },
	{ 0x354e16f1, "nf_conntrack_helpers_register" },
	{ 0x122c3a7e, "_printk" },
	{ 0x87a21cb3, "__ubsan_handle_out_of_bounds" },
	{ 0x76bfda42, "nf_conntrack_helpers_unregister" },
	{ 0x114e3d2b, "module_layout" },
};

MODULE_INFO(depends, "nf_conntrack");


MODULE_INFO(srcversion, "9A2C91AA22115C3D118EE49");
